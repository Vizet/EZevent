<template>
  <div id="app">
    ...
    <!-- Левое меню. содержащее ссылки на разделы -->
    <div class="side-menu">
    ...
    <!-- Блок в котором отображаются компоненны с контентом страниц -->
...
export default {
  ...
  data () {
    return {
      //dno: 'rak', // просто тестовое значение?
      //arr: [1, 2], // используется в методе me
      events: [], // это надо перенести во Vuex
      requests: [],
      //eventFormFlag: false, // не нашёл на что влияет этот флаг... Это от старого "роутера"?
      //statFlag: false, // не нашёл на что влияет этот флаг... Это от старого "роутера"?
      //reqStat: [] // хранилище для данных статистики. На данный момент не используется...
    }
  },
  mounted: function(){
      // Получаем данные о мероприятиях из БД
      this.getGroupData();
  },
  methods: {
    /*me: function(){ // зачем нужен этот метод? На данный момент он ни на что не влияет
      this.arr.push('');
    },*/
    /*userLogin: function(){ // Отладочный метод?
      axios({
        method:'get',
